...
select
a.age, a.sexe, a.zone, a.qm, b.valeur
from &&prefix._&&idep._quotient_menp a, cb_hypothese b, &&prefix._&&idep._cube c
...
create table &&prefix._&&idep._quotient_menj as
select a.age, a.sexe, c.annee, a.zone,
    decode(c.valeur, 0, 0, 1, 1, 
	  decode
	  (
	  nvl((a.qm*(1-a.valeur)*c.valeur+(1-a.qm)*a.valeur*(1-c.valeur)),0),0,0,
	  nvl((a.qm*(1-a.valeur)*c.valeur),0)/(a.qm*(1-a.valeur)*c.valeur+(1-a.qm)*a.valeur*(1-c.valeur))
	  ) 
    ) qm
from  &&prefix._&&idep._quotient_meni a, cb_hypothese c
