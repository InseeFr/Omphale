...
select &&annee_ref. annee_ref, &&annee_hor. annee_horizon, &&annee_ref. + 5*ceil((&&annee_hor. - &&annee_ref. )/5) annee_hor,
ceil((&&annee_hor. - &&annee_ref. )/5) nb_pas
from dual
;
drop table &&prefix._&&idep._pas;
create  table &&prefix._&&idep._pas as
select a.*, rownum pas_cur, annee_ref+5*rownum-5 annee_pas, 0 a_faire from &&prefix._&&idep._nb_pas a, annees b
