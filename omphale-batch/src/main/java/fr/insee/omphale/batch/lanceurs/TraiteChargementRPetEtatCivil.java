...
public class TraiteChargementRPetEtatCivil implements Batch{
    ...
    public int lancementDuBatch() {
        ...
        try {
            ...
            CacheDaoManager.beanRapport.erreurProjection="Une erreur s'est produite lors de la recherche du fichier à charger ";
        ...
        try {
            ...
            while (plugins.hasNext()) {
                ...
                try {
                    ...
                    CacheDaoManager.beanRapport.erreurProjection="Impossible d'instancier le plugin "+e.getMessage();
                    return STATUS_ERROR_CONFIG;
                    ...
                    CacheDaoManager.beanRapport.erreurProjection="Une erreur inattendue s'est produite lors de l'execution du plugin "+e.getMessage();
                    return STATUS_ERROR_CONFIG;
                }
            }
        } catch (ClasspathException e1) {
            CacheDaoManager.beanRapport.erreurProjection="Impossible d'inspecter les plugins "+e1.getMessage();
            return STATUS_ERROR_CONFIG;
        }
        if (!traite) {
            CacheDaoManager.beanRapport.erreurProjection="Fichier csv sans processus associe :"+laClasse+".csv";
            if (!OmphaleGestionFile.deplacerForce(new File(repErreur+File.separator+laClasse+".csv"), new File(repErreur+File.separator+laClasse+".csv")))
            {
                CacheDaoManager.beanRapport.erreurProjection+="\r\nFichier impossible à deplacer dans erreur : " + laClasse+".csv";
               
            }
            return STATUS_ERROR_METIER;
        }
        CacheDaoManager.beanRapport.erreurProjection+=" Le Fichier " + laClasse+".csv a été traité avec succès";
     ...
     * certains chargement doivent s'effectuer absolument à la fin d'autres procèdures de chargement
    ...
    private static String getFichier() throws Exception{
        ...
        {
            throw new Exception("Fichier impossible à deplacer dans aCharger : " + leFic+".csv");
