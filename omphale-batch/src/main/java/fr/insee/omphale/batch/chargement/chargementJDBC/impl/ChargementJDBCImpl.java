...
import fr.insee.omphrefo.batch.lanceurs.Colonne;
import fr.insee.omphrefo.batch.lanceurs.Table;

public class ChargementJDBCImpl {

	   public void chargementFichierVersUneTable(IConnectionManager cm, String nomTable, String emplacementFichier) throws Exception{
	    	
	    		Connection connection = cm.getConnection(OmphaleBatchConfig.nomConnexion);

	    		PreparedStatement st = connection.prepareStatement("SELECT * from "+ nomTable.toLowerCase());
	    		...
	    		File file = new File(emplacementFichier);
	    ...
	    public Map<Integer, String> verificationEtInitialisationEnteteEnVueDeLaRequete(Table table, String entetesFichier) throws Exception{
			...
			while (it.hasNext()){
				...
				// on vérifie qu'une entête du fichier est égale au nom de la colonne courante
				...
				if(!estEgale){
					throw new Exception("La colonne " + nomColonne + " est absente du fichier.");
				}
